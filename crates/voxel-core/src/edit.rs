...
/// know about the history at all: build a transaction, write through it, commit.
pub struct Transaction<'a> {
    model: &'a mut VoxelModel,
    label: &'static str,
    edits: Vec<Edit>,
}
...
impl<'a> Transaction<'a> {
    ...
    pub fn set(&mut self, x: i32, y: i32, z: i32, value: u8) {
        if !self.model.contains(x, y, z) || self.model.get(x, y, z) == value {
            return;
        }
        let before = self.model.set(x, y, z, value);
...
impl History {
    ...
    ///
    /// Returns whether anything changed. An empty transaction is discarded —
    /// pushing it would leave the user with an undo that visibly does nothing,
    /// which reads as a broken undo rather than an empty one.
    pub fn edit(
        &mut self,
...
mod tests {
    ...
    fn a_write_that_changes_nothing_is_not_an_undo_step() {
        ...
        assert!(!h.edit(&mut m, "paint", |tx| tx.set(0, 0, 0, 4)));
        assert!(!h.edit(&mut m, "build", |tx| tx.set(99, 0, 0, 1)));
    ...
    fn editing_after_an_undo_drops_the_redo_stack() {
        ...
        h.edit(&mut m, "a", |tx| tx.set(0, 0, 0, 1));
        h.undo(&mut m);
        assert_eq!(h.redo_depth(), 1);
        h.edit(&mut m, "b", |tx| tx.set(1, 0, 0, 1));
